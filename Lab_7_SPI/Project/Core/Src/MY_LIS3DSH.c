...
#define HUGE_TILT_RANGE 800
#define LITTLE_TILT_RANGE 13
//Library variables
//1. SPI handle
...
void transmitFloatCooridinate(UART_HandleTypeDef *huart, float value, const char *label) {
	char buffer[50];
	int len = snprintf(buffer, sizeof(buffer), "%s: %f\r\n", label, value);
	HAL_UART_Transmit(&huart3, (uint8_t*) buffer, len, HAL_MAX_DELAY);
}

void transmitSeparator(UART_HandleTypeDef *huart) {
	const char *separator = "--------------------------------\r\n";
	HAL_UART_Transmit(huart, (uint8_t*) separator, strlen(separator),
			HAL_MAX_DELAY);
}

void transmitHugeTiltWarning(UART_HandleTypeDef *huart) {
	char message[] = "Warning! Huge tilt detected!\n\r";
	HAL_UART_Transmit(&huart3, (uint8_t*) message, sizeof(message),
			HAL_MAX_DELAY);
	transmitSeparator(&huart3);
}

void transmitCoordinates(UART_HandleTypeDef *huart,
		LIS3DSH_DataScaled coordinatesStruct) {
	transmitFloatCooridinate(&huart3, coordinatesStruct.x, "Coordinates of x");
	transmitFloatCooridinate(&huart3, coordinatesStruct.y, "Coordinates of y");
	transmitFloatCooridinate(&huart3, coordinatesStruct.z, "Coordinates of z");

	transmitSeparator(&huart3);

	if (abs(coordinatesStruct.x) > HUGE_TILT_RANGE || abs(coordinatesStruct.y) > HUGE_TILT_RANGE) {
		transmitHugeTiltWarning(&huart3);
		//	transmitSeparator(&huart3);
	}
}

void setLedsPWM(LIS3DSH_DataScaled coordinatesStruct) { // MAGIC NUMBERS
	if (coordinatesStruct.x > LITTLE_TILT_RANGE) {
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_3, coordinatesStruct.x);
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, 0);
	} else if (coordinatesStruct.x < -LITTLE_TILT_RANGE) {
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, abs(coordinatesStruct.x));
	...
	if (coordinatesStruct.y > LITTLE_TILT_RANGE) {
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, coordinatesStruct.y);
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, 0);
	} else if (coordinatesStruct.y < -LITTLE_TILT_RANGE) {
		__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, abs(coordinatesStruct.y));
