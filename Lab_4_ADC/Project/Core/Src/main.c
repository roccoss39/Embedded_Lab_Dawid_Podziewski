...
const float SupplyVoltage = 3.04; // [checked by multimeter]
const float ResolutionADC = 4095.0;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
	...
	if (hadc->Instance == ADC1) {
		...
		TVoltage = (SupplyVoltage * TemperatureADC) / ResolutionADC;
	...
	if (hadc->Instance == ADC2) {
		...
		uint16_t ResistanceADC = 0;
		ResistanceADC = HAL_ADC_GetValue(&hadc2);
		Voltage = (SupplyVoltage * ResistanceADC) / ResolutionADC;
		...
		if (Voltage < 0.1)
			...
			__HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1,(uint32_t )(Voltage * 330));
